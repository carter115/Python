...
for i in filter(notlt0,s):
    print(i)
# 定义一个notLt0的方法，该方法用于判断元素是否大于等于0，是则返回True，否则返回False。而filter则会将列表s的每一个元素作为参数调用notLt0方法，最后将所有返回True的元素重新组装成一个list返回。
...
'''
# 闭包就是你调用了一个函数A，这个函数A返回了一个函数B给你。这个返回的函数B就叫做闭包。你在调用函数A的时候传递的参数就是自由变量。举个例子：
def func(name):
    def inner_func(age):
